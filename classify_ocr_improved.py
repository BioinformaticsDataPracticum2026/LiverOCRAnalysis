#!/usr/bin/env python3
"""
Pipeline for classifying open chromatin regions (OCRs) as promoters/enhancers
and comparing regulatory states across species.
...
from pybedtools.tricycle import run_and_move
...
def classifyOcrPromotersEnhancers(
    ...
    logger.info(f"Processing OCRs: {ocr_bed_path}")
    
    try:
        #Load BED files
        ocr = BedTool(str(ocr_bed_path))
        tss = BedTool(str(tss_bed_path))
        ...
        #Annotate nearest TSS with distance (last column)
        ...
        ocr_promoters = ocr_with_genes.filter(
            lambda x: int(x[-1]) <= promoter_distance
        ).saveas(promoters_path)
        logger.info(
        ...
        ocr_enhancers = ocr_with_genes.filter(
            lambda x: int(x[-1]) > promoter_distance
        ).saveas(enhancers_path)
        logger.info(
            f"Classified {len(ocr_enhancers)} enhancers (> {promoter_distance} bp): "
            f"{enhancers_path}"
        )
        
        logger.info(f"Finished processing {ocr_bed_path}")
...
def classifyConservedRegions(
    ...
    logger.info(f"Processing conserved regions: {conserved_bed_path}")
    
    try:
        conserved = BedTool(str(conserved_bed_path))
        tss = BedTool(str(tss_bed_path))
...
def findSharedElements(
    ...
    This function finds mapped OCRs from one species that intersect with native
    ...
    logger.info(f"Finding shared elements between {mapped_file_path} and {native_file_path}")
    
    try:
        mapped = BedTool(str(mapped_file_path))
        native = BedTool(str(native_file_path))
        ...
        # Find intersecting regions (u=True keeps only regions that intersect)
